...
struct geopoint {
	...
	friend bool operator<(const geopoint & p, const geopoint & q) {
		if (p.lat < q.lat)
			return true;
		if (p.lon < q.lon)
			return true;
	...
	friend bool operator==(const geopoint & p, const geopoint & q) {
		return p.lat == p.lat and p.lon == q.lon;
	}
...
private:
	...
	std::map<uint64_t, geonode> nodes;
	std::map<uint64_t,std::set<uint64_t>> adjacents;
