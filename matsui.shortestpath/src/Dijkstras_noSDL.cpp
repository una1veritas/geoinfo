...
vector<long long> findNodesInZone(const unordered_map<long long, DataEntry>& graph, long long center_of_zone, double zone_radius) {
    vector<long long> nodes_in_zone;

    for (const auto& entry : graph) {
        double distance = calculateDistance(graph.at(center_of_zone).lat, graph.at(center_of_zone).lon, entry.second.lat, entry.second.lon);
...
bool is_in_circle(const unordered_map<long long, DataEntry>& graph,
		const long long & point_id,
		const long long & center_id,
		const double & radius) {
	double dist = calculateDistance(graph.at(center_id).lat, graph.at(center_id).lon,
			graph.at(point_id).lat, graph.at(point_id).lon);
	if (dist <= radius)
		return true;
    return false;
}
...
// 優先順位付きキューの無しのダイクストラ法で最短距離と経路を求める関数
void dijkstraToZone(
    ...
    for (const auto& entry : graph) {
    	if (is_in_circle(graph, graph[entry.first]), center_of_outer, outer_radius) {
    ...
    while (i < graph.size()) {
        ...
        // 次の安全地帯内のノードに到達したらwhileループから抜ける
        if ((calculateDistance(graph.at(current_node).lat, graph.at(current_node).lon,
            graph.at(center_of_next_zone).lat, graph.at(center_of_next_zone).lon) <= next_zone_radius)) {
...
// 優先順位付きキューを用いたダイクストラ法で最短距離と経路を求める関数
void dijkstraToZone_pq(
    ...
    while (i < graph.size()) {
        ...
        for (long long neighbor : graph.at(current_node).adj) {
            ...
            // 暫定距離よりも短くなる場合は、dist[neighbor]とprev[neighbor]を更新し、キューに追加
            if (dist[current_node] + cost < dist[neighbor]) {
                dist[neighbor] = dist[current_node] + cost;
                prev[neighbor] = current_node;
...
int main() {
    string filename = "map-central-iizuka.geo";
    char delimiter = ',';
    ...
    // 次の安全地帯の中心と半径
    long long center_of_next_zone = 1540568884; // 九工大のKITの植物アートのところ 1540568884,33.652744,130.6719787
    ...
    */
    // 次の安全地帯の中心の情報の出力
    if (center_of_next_zone != -1 && graph.find(center_of_next_zone) != graph.end()) {
        cout << "○次の安全地帯の中心　ID:" << center_of_next_zone << "("
            << graph[center_of_next_zone].lat << ", " << graph[center_of_next_zone].lon << ")"
            << endl;
    }
    ...
    int size_of_nodes_inside_the_current_zone = nodes_inside_the_current_zone.size();

    // 1回目の実行時間計測
    auto start_time_1 = chrono::high_resolution_clock::now();

    dijkstraToZone(graph, startNode, center_of_next_zone, next_zone_radius);
    ...
    size_of_nodes_inside_the_current_zone = nodes_inside_the_current_zone.size();

    // 2回目の実行時間計測
    auto start_time_2 = chrono::high_resolution_clock::now();

    dijkstraToZone_pq(graph, startNode, center_of_next_zone, next_zone_radius);
